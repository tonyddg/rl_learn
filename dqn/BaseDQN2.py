...
class BaseDQN:
    ...
    def  _update_epsilon(self, action_take_times: int):
        ...
        self.epsilon = \
            ...
            * np.exp(- action_take_times / self.hparams.action_take_time_decay) # 指数规律 epsilon
    ...
    def take_action_single(self, state: np.ndarray, is_update_epsilon: bool = True) -> np.ndarray:
        '''
    ...
    def _sync_target_network(self):
        '''
        使用加权法, 更新 Target Network 参数
        '''
        q_network_state_dict = self.q_network.state_dict()
        target_network_state_dict = self.target_network.state_dict()

        for key in q_network_state_dict.keys():
            target_network_state_dict[key] = \
                q_network_state_dict[key] * (1 - self.hparams.tau) + q_network_state_dict[key] * self.hparams.tau
